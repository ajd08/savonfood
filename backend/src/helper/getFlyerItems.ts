...
const flyer_scraper = async (
    ...
    for (let i = 1; i < numItems; i++) {
        ...
        for(let j=0;j<same_price_items.length;j++) {
            console.log(same_price_items[j]);
        }
...
const nofrills_flyer_crawler = async () => {
    ...
    flyer_scraper(nofrills, flyer_link);
};

const metro_flyer_crawler = async () => {
    const browser = await puppeteer.launch({ headless: true });
    const page = await browser.newPage();
    ...
    flyer_scraper(metro, flyer_link);
};

const food_basics_crawler = async () => {
    const browser = await puppeteer.launch({ headless: true });
    const page = await browser.newPage();
